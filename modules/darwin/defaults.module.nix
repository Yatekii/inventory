{ ... }:
{
  ...
  # Restore Spotlight's ⌘Space / ⌥⌘Space hotkeys (keys 64/65) in case a
  # previous deploy disabled them to hand ⌘Space to a third-party launcher.
  # Keeping the keys here pinned to `enabled = true` overwrites the stale
  # "false" that macOS would otherwise persist in com.apple.symbolichotkeys.plist.
